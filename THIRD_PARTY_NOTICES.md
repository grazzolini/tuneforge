...
- **Notes:** These packages are locked only for the optional `advanced-beats` extra and packages built with `--beat-this`, and reuse the existing PyTorch, torchaudio, einops, and soxr stack already present in `apps/backend/uv.lock`. The standard backend dependency set still uses Built-in Beat Analysis by default.
