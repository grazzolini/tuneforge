...
- **Notes:** The standard Linux Flatpak uses the locked PyTorch runtime. The full Flatpak profile swaps in the legacy NVIDIA CUDA 12.6 PyTorch and torchaudio wheels for older supported NVIDIA GPUs.
...
- **Notes:** The full Linux Flatpak profile redistributes the CUDA 12.6 runtime wheel set pulled by PyTorch 2.6.0+cu126, including cuBLAS, cuDNN, cuFFT, cuSOLVER, cuSPARSE, NCCL, NVRTC, NVTX, and related runtime libraries. These wheels are large and are included only in the full local/dev Flatpak profile.
...
- **Notes:** No GPL/AGPL/SSPL package is present in the locked optional Advanced Chords Python tree. Packaging risk is still higher than the built-in backend because TensorFlow, h5py/HDF5, gRPC, and related packages ship native binary wheels and larger transitive notice sets. Complete a fresh machine-readable inventory before enabling this stack in the standard packaged desktop build.

### Advanced Beat Analysis transitive runtime stack

- **Primary new packages:** beat-this (MIT) and rotary-embedding-torch (MIT).
- **Notes:** These packages are locked only for the optional `advanced-beats` extra and the full Linux Flatpak profile, and reuse the existing PyTorch, torchaudio, einops, and soxr stack already present in `apps/backend/uv.lock`. The standard backend dependency set still uses Built-in Beat Analysis by default.
