use crate::mobile_ffmpeg::{render_audio, render_whisper_audio, AudioOutputFormat};
use crate::native_audio::decode::{
    probe_mobile_durable_audio, read_mobile_audio, write_mono_pcm_wav,
};
use crate::native_audio::whisper_alignment::{
    partition_aligned_words, split_aligned_words, validate_aligned_token_coverage,
    validate_aligned_word_boundaries, AlignedTextToken,
};
use android_system_properties::AndroidSystemProperties;
use tauri_plugin_fs::{FilePath, FsExt, OpenOptions};
use whisper_rs::{
    install_logging_hooks, DtwMode, DtwModelPreset, DtwParameters, FullParams, SamplingStrategy,
    WhisperContext, WhisperContextParameters, WhisperSegment,
};
...
pub(crate) use lyrics::request_lyrics_job_cancellation;
...
pub fn mobile_capabilities(app: AppHandle) -> Result<MobileCapabilities, String> {
    ...
    let whisper_model_status = crate::native_audio::whisper_model::status();
    let is_emulator = is_android_emulator();
    let beat_this_model_status = crate::native_audio::beat_this::android_model_status();
    let crema_model_status = crate::native_audio::crema::android_model_status();
    Ok(MobileCapabilities {
        ...
        crema_available: crema_model_status != "unavailable",
        crema_model_status,
        whisper_available: whisper_model_status == "ready",
        whisper_model_status,
        stem_separation_available: false,
        generation_testing_available: generation_testing_available(is_emulator),
        max_recommended_model: Some("large-v3-turbo"),
